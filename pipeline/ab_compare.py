...
def main():
    ...
    for entry in manifest:
        ...
        # version-B: token-aligned consensus
...
def write_report(results):
    ...
    if a:
        ...
        lines.append("")
        lines.append("#### Why CER and token-accuracy disagree (read this before quoting CER)")
        lines.append("")
        lines.append("On several pages version-B fixes real word errors yet its **windowed CER is "
                     "flat or slightly worse** (e.g. `era1899_p162`: token errors 8→3, *zero* "
                     "regressions, but CER 0.93%→3.79%). That is **not** a quality regression — it "
                     "is an artifact of the CER measurement:")
        lines.append("")
    ...
    lines.append("")
    lines.append("| Page | Era | Engines | Method | A CER% | B CER% | delta | fixed | regr | conf |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|")
    for r in pp:
        lines.append(
