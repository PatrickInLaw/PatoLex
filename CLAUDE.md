...
**Pipeline stack:** The **historical corpus build (cc002 decision) uses Python (OCR/parse — Tesseract 5 + docTR + Surya, merged by token-aligned majority-vote consensus in `pipeline/consensus.py`; PyMuPDF for the born-digital text-layer fast-path) + TypeScript/Drizzle (ingest against the same schema)**. *(The OCR engines are Tesseract+docTR+Surya — NOT "qwen2.5vl ensemble," which earlier docs misstated; VLMs were only a design idea for verification disagreement-vectors, never an OCR engine in the running pipeline.)* This is the proven, working toolchain for the one-time 1850-forward reconstruction. The originally-specified **C# (.NET 8 LTS) pipeline (AngleSharp, PdfPig, Tesseract.NET, Dapper) is DEFERRED** — reserved for an ongoing modern-era crawler/worker if/when wanted, not the historical ETL.
...
- **Corpus data root (OCR/parse artifacts + source PDFs):** the historical-build scratch corpus lives on the 5090 at **`C:\PatoLex-scratch`** — relocated 2026-06-19 out of the `patolex` user profile so any account can read/write it; resolved machine-wide via the **`PATOLEX_LOCATION_ROOT`** env var (`config.py` honors it). Source PDFs incl. every printed `*_Index.pdf` (1850→modern) are under `C:\PatoLex-scratch\chief-clerk-archive\`. This is the OCR/parse **file** corpus — distinct from the Postgres DB above. (Older docs/scripts referencing `C:\Users\patolex\PatoLex-scratch` are stale; resolve via the env var.)
...
| 2026-06-20 | cc015: Recorded the **corpus scratch-root relocation** — the OCR/parse corpus + source PDFs now live at **`C:\PatoLex-scratch`** (moved out of the `patolex` user profile; resolved via `PATOLEX_LOCATION_ROOT`/`config.py`), and every printed `*_Index.pdf` (1850→modern) is now local on the 5090. (Same session fixed the 1854 dual-series parse: contents-anchored, 174/174, Hans-SOUND — see lessons.) |
