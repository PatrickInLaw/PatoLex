...
| **Session laws (Chief Clerk)** | CA Assembly Chief Clerk archive | **Every statute volume 1850-2008**, 653 PDFs (fills the catalog's post-1966 Statutes gap) | **Mixed format — see §1d THREE-TIER model.** Volumes through ~1999 are OCR-required (image-only 1850-1996; **1997-1999 have a text layer but it is garbled mojibake → OCR**); the clean modern tail (**2000-2008**, verified by extraction of 65 vols / 8,291 chapters) is **born-digital with a clean text layer → direct text extract, no OCR.** | Free direct HTTP, predictable URL pattern. 1993 = 5 vols. |
...
## 1d. The THREE-TIER corpus model — which extraction method per era (cc002, 2026-06-02)

**Correction (2026-06-02):** an earlier version of this file labeled the *entire* 1850-2008 Chief Clerk range as "Image-only PDF (no text layer)." That is **wrong for the modern tail.** A second correction (also 2026-06-02) moved the clean-text boundary from ~1997 to **2000**: the 1997-1999 volumes were assumed extractable but are not (garbled mojibake — see tier (a)). The corpus splits into **three tiers**, each with a different extraction method. This is the single most important operational fact for planning the build — it bounds the OCR campaign.

The Chief Clerk backbone is **653 PDFs, 1850-2008** (258 body volumes, 413,987 body pages — per-volume counts in `C:\Users\PatrickKolasinski\PatoLex-scratch\corpus_page_counts.csv`).

| Tier | Era | Format | Method | Verified |
|------|-----|--------|--------|----------|
| **(a) OCR-required** | **1850 – ~1999** | **Either** image-only scanned PDF, **no text layer** (1850-1996) **or** born-digital PDF whose text layer is **garbled mojibake and unextractable** (1997-1999) | **OCR** (multi-engine consensus pipeline; see §2 + `pipeline/`) — or glyph re-mapping for 1997-1999 | 1850 (`acquire-ocr-1850-run.log`: "480 pages, image-only, PixEdit scan"), 1871-72 (`resource-1872-pure-run.log`), **1995 still image-only**, and **1997/1998/1999 (17 vols) have a text layer that returns junk** — body set in broken subset fonts (`PSOwstdutch` etc.) with missing/broken ToUnicode CMap → 0 chapters extract (`born-digital-3060-run.log`, 2026-06-02). |
| **(b) Born-digital Chief Clerk (clean)** | **2000 – 2008** | Born-digital PDF with a **clean embedded text layer** | **Direct text extraction**, NO OCR (`pipeline/5080/parse_born_digital_prod.py`, the productionized extractor; prototype was `parse_born_digital.py`) | 65 vols (2000-2008) extracted cleanly → **8,291 chapters** (8,290 confident), continuous per-year numbering, zero OCR (`born-digital-3060-run.log`, 2026-06-02). 2000=1092, 2001=979, 2002=1175, 2003=926, 2004=956, 2005=729, 2006=910, 2007=750, 2008=774 statutes/year. |
| **(c) Modern structured** | **1989/1994 – present** | leginfo PUBINFO bulk data — born-digital **CAML XML** + tab-delimited `.dat` | **Bulk import + reconstruct point-in-time backward** from chaptered bill XML, validated against the current snapshot. NOT OCR, NOT the Chief Clerk PDFs. | See `DATA_SOURCES.md` §1-§4 (the authoritative modern-era doc). |

**The OCR-required / clean-born-digital crossover is 2000 (verified, not estimated).** Evidence (`born-digital-3060-run.log`, 2026-06-02): 2000-2008 (65 vols) extract cleanly via `parse_born_digital_prod.py`; 1997, 1998, 1999 (17 vols) have a text layer but it is garbled mojibake (broken subset fonts / missing ToUnicode CMap) and yields 0 chapters. So the **OCR ceiling is ~1999**, and clean direct text-extraction starts at **2000**. (An earlier draft put the boundary at ~1997 / OCR ceiling ~1996; that is superseded — the 1997-1999 born-digital volumes are *not* extractable.)

**Consequence for the OCR campaign:** for the POC, OCR of the Chief Clerk session laws is bounded on the modern end at **~1993-94, NOT 2008**, because the structured XML channel takes over there:
1. From tier (c): leginfo PUBINFO carries machine-readable chaptered-bill XML back to the **1993-94 session** (POC floor ≈ Jan 1, 1994 — `DATA_SOURCES.md` §3). The modern point-in-time text is **reconstructed backward from that XML**, not OCR'd.
2. From tier (b): the **2000-2008** Chief Clerk PDFs are clean born-digital and need only `parse_born_digital_prod.py`, never OCR.

So for the POC, OCR (tier a) only has to cover **1850 → ~1993** of the Chief Clerk session laws; the years after are served by the structured XML channel (c) and/or, for 2000-2008, direct text-extract (b). **Do not OCR the *clean* born-digital tail (2000-2008).**

**Caveat — 1997-1999 are OCR volumes, not extractable.** Although 1997-1999 are born-digital, their text layer is garbled (see tier (a)), so they fall under OCR, not direct extraction. They sit *above* the ~1993-94 POC XML floor, so the POC reconstruction does not need them. **But if the OCR queue is ever extended past ~1975 (e.g., to fill the Chief Clerk image era forward), 1997-1999 must be included as OCR volumes** — they were previously assumed extractable and would otherwise be silently skipped between the clean 2000-2008 extract and the image-only ≤1996 OCR set.

**Statute-format eras** (drives which parser path runs — see `docs/80_PROJECT_HISTORY/MODERN_STATUTE_FORMAT_2026-06-02.md` for the modern format spec):
- **1850 – ~1910:** single-volume per year; `CHAP. N / An Act … / do enact / Approved <date>` (Roman-numeral chapters; OCR-fuzzy in 1850s-1870s long-s scans).
- **~1915+:** multi-volume per year (`Vol1_Chapters` .. `VolN`), **chapters numbered continuously across the volumes of a year** — the parser must roll the volumes up into one chapter stream.
- **Modern (the chaptered Statutes volumes generally; cleanly extractable born-digital from 2000+):** `Approved by Governor <date> / Filed with Secretary of State <date>`, Arabic-numeral chapters, and **NO bill-number markers** (`[Senate Bill No.]` confirmed absent in the chaptered sections of 2001 & 2008). Note the same format applies to 1997-1999 volumes, but those must be reached via OCR (garbled text layer), not direct extraction.
...
| Date | Change |
|------|--------|
| 2026-06-02 | cc002 (boundary correction): Moved the clean-text born-digital boundary from ~1997 to **2000** per `born-digital-3060-run.log` — 2000-2008 (65 vols / 8,291 chapters) extract cleanly via `parse_born_digital_prod.py`, but 1997-1999 (17 vols) have garbled-mojibake text layers (broken subset fonts / missing ToUnicode CMap) → **OCR-required**. Renamed tier (a) to **OCR-required 1850-~1999** (image-only ≤1996 + garbled born-digital 1997-1999), tier (b) to **clean born-digital 2000-2008**; set crossover = 2000 and OCR ceiling = ~1999; added the 1997-1999 OCR-queue caveat. No surviving ~1997/~1996 boundary claims. |
| 2026-06-02 | cc002 (doc rewrite): Reconciled to as-built schema — recodification events are typed **`lineage_edge`** rows (no first-class `recodification` table); corrected the §3 date prose from `enacted_date` to the as-built `chaptered_date`/`effective_date`/`operative_date` trio. |
| 2026-06-02 | cc002 (doc remediation): Added §1d THREE-TIER corpus model (image-only → OCR; born-digital ~1997+ → text-extract via `parse_born_digital.py`; leginfo PUBINFO XML → reconstruct). Corrected the §1 table cell that wrongly labeled all 1850-2008 Chief Clerk as image-only. Pinned image-only/born-digital crossover at ~1997 (exact volume TBD) and the OCR ceiling at ~1993-94, not 2008. Added the statute-format eras. Cross-referenced `DATA_SOURCES.md` (modern) + `MODERN_STATUTE_FORMAT_2026-06-02.md`. |
