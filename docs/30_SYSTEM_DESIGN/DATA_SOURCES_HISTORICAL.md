...
| **Session laws (Chief Clerk)** | CA Assembly Chief Clerk archive | **Every statute volume 1850-2008**, 653 PDFs (fills the catalog's post-1966 Statutes gap) | **Mixed format — see §1d THREE-TIER model.** Early volumes are image-only PDF (no text layer → OCR); the modern tail (~1997+, verified 2001 & 2008) is **born-digital with a clean text layer → direct text extract, no OCR.** | Free direct HTTP, predictable URL pattern. 1993 = 5 vols. |
...
**Correction (2026-06-02):** an earlier version of this file labeled the *entire* 1850-2008 Chief Clerk range as "Image-only PDF (no text layer)." That is **wrong for the modern tail.** The corpus splits into **three tiers**, each with a different extraction method. This is the single most important operational fact for planning the build — it bounds the OCR campaign.
...
|------|-----|--------|--------|----------|
| **(a) Image-only** | **1850 – ~1996** | Image-only scanned PDF, **no text layer** | **OCR** (multi-engine consensus pipeline; see §2 + `pipeline/`) | 1850 (`acquire-ocr-1850-run.log`: "480 pages, image-only, PixEdit scan"), 1871-72 (`resource-1872-pure-run.log`), and **1995 still image-only**. |
| **(b) Born-digital Chief Clerk** | **~1997 – 2008** | Born-digital PDF with a **clean embedded text layer** | **Direct text extraction**, NO OCR (`pipeline/5080/parse_born_digital.py`) | 2001_Vol1 (1718 pp) and 2008_Vol1 (1652 pp) confirmed born-digital; `parse_born_digital.py` pulled 227 chapters from 2008_Vol1 at 100% confidence, continuous numbering, zero OCR (`modern-parser-run.log`, 2026-06-02). |
...
**The image-only/born-digital crossover is ~1997, exact volume TBD.** The pinned evidence is: image-only at least through 1995; born-digital by 2001. The precise crossover volume (1996 vs 1997) has **not yet been characterized** — when planning the OCR campaign, treat the OCR ceiling as **~1996, to be tightened to the exact volume before reaching it.**

**Consequence for the OCR campaign:** it is bounded on the modern end at **~1993-94, NOT 2008.** Two independent reasons converge there:
...
2. From tier (b): the late-1990s/2000s Chief Clerk PDFs are born-digital and need only `parse_born_digital.py`, never OCR.

So OCR (tier a) only has to cover **1850 → ~1993** of the Chief Clerk session laws; the years after are served by direct text-extract (b) and/or the structured XML channel (c). **Do not OCR the born-digital tail.**
...
- **Modern (~1997+ born-digital and the chaptered Statutes volumes generally):** `Approved by Governor <date> / Filed with Secretary of State <date>`, Arabic-numeral chapters, and **NO bill-number markers** (`[Senate Bill No.]` confirmed absent in the chaptered sections of 2001 & 2008).
