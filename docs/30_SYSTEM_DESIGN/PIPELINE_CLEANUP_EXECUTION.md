...
- [ ] **Step A — de-dup core** (`ocrcorrect/dictionary.py` + `edits.py`, injectable). GATE.
...
## Step C — Scratch de-gating
Move corpus data `C:\Users\patolex\PatoLex-scratch` → `C:\PatoLex` (only real volume, 507 GB free), grant
Users full control, so any login reaches it without the patolex account. De-hardcode the path: read from a
single `PATOLEX_SCRATCH` env/config (default `C:\PatoLex`) instead of the baked-in string in every script.
Junction the old path during transition so nothing breaks. GATE → commit.
