...
- **NOT real (parser artifact):** **1865-66 (index 650 vs oracle 280)** — Hans CRITICAL C1/M4 (max-run not contiguous-from-1, no END-marker for subject/amendment indexes); **prior 280 stands.** 1858/1891 (+2) = noise (classify band too tight at ±1; widen to ±2).
- **STRUCTURAL: the modern era (≈1905+) is NO_INDEX because the OCR bundles begin at the statute body — the printed CONTENTS pages were never bundled.** Index re-derivation only works for the early single-vol era (≈1858–1903), which is exactly where the oracle errors are. To cross-check the modern denominator you must OCR the printed CONTENTS pages separately (they're in the source scans, not the bundles).
