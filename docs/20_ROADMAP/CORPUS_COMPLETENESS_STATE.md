...
| (iii) oracle-suspect | 12 | low-N chaptered-era "regular" oracle rows — **VERIFIED CORRECT** (not errors), see below |
| (iv) floor over-extraction | 687 phantom / 10 sessions | precision/"garbage", NOT coverage |
- **(iii) and (iv) are the SAME artifact — and the oracle is NOT wrong.** VERIFIED 2026-06-17 against the cited clerk.assembly.ca.gov archive: **1883 = ch 1–23, 1887 = ch 1–51 are CORRECT** (not undercounts); the even-year Budget rows (1964=1, 1958=10…) are correct too. The high floors against them are NOT an oracle problem — they are: (a) the **bundling artifact** (real extra-session acts mapped to the budget-session row → a measurement/labeling fix: map a bundled volume to ALL the sessions it covers, NOT garbage), and (b) genuine **FLOOR over-extraction** (code-amendments/resolutions parsed as statute chapters — 1883-84 Code vol, 1865-66 class → reclassify confident→flagged). **The fix is in the floor/measurement, NOT the oracle. No oracle edits are warranted from this analysis.**
