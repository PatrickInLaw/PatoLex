...
## 3h. INDEX RE-DERIVATION — oracle reconciliation from printed TOCs (2026-06-17, `rederive_index_counts.py`, Hans-reviewed)
Per Patrick: re-derive each volume's chapter count from its OWN printed index as a SECOND authoritative signal and COMPARE to the oracle — overwrite nothing. Tool: `pipeline/analysis/rederive_index_counts.py` (write-safe; outputs `_index_rederivation.tsv`/`.md` + per-volume `index_chapter_map.json` on the 5090). Result over 222 volumes: **MATCH 12, ORACLE_LOW 6, ORACLE_HIGH 1, NO_INDEX 203.** Validated on controls (1885=169, 1889=290, 1895/1897/1899/1903 MATCH; 1887 flagged).
- **Hans-confirmed REAL oracle undercounts:** **1887 (51→~188), 1883-84 (23→~96)** — act on these (with a chapter_map cross-check); 1863 (476→538) plausible, spot-check.
- **~~NOT real (parser artifact): 1865-66 (index 650 vs oracle 280) — Hans C1/M4; prior 280 stands.~~ ⚠ SUPERSEDED 2026-06-17 (cc013) — 1865-66 is a REAL undercount.** Direct read of the volume's own printed index (authoritative internal source, lesson #4) shows a **continuous run of ~380 real `An Act` titles climbing chapter 1→650**, ending cleanly at the resolutions marker — **no amendment/subject-index pollution; Hans's C1/M4 failure mode is NOT present here.** Index distinct ≥488 (coverage ≥0.75) and the parsed floor already holds 463 distinct chapters — both impossible under a true 280. ⇒ **oracle 280 is a SEVERE UNDERCOUNT (~650), same signature as 1887/1883; the tool's 650 was correct.** The 2026-06-16 "1865-66 confirmed correct" + the parser-artifact tier were the **1887 flip-flop repeating** (clerk web index trusted over the printed volume). Other ORACLE_LOW re-read (cc013): **1863 476→~538 REAL (+62)**; **1858 358→360** and **1891 280→282** = +2 noise (1891 modal_year=1872 was an old-code-citation red herring). All flagged for Patrick's oracle decision — **do not overwrite.**
- **~~STRUCTURAL: modern era NO_INDEX → must OCR the printed CONTENTS pages separately to cross-check the modern denominator.~~ ⚠ SUPERSEDED 2026-06-17 (cc013) — modern era does NOT need the CONTENTS pages.** (a) **We already hold every modern source PDF** — `chief-clerk-archive/` has 211 PDFs spanning 1861–2000 incl. the full 1905–1999 range; the bundles begin at `CHAPTER 1.`, so the absent CONTENTS pages are an OCR-*scope* matter, **not an acquisition gap.** (b) **The modern body is SELF-INDEXING** — chapters run sequentially, so the contiguous-from-1 top of the body's `CHAPTER N` headers IS the count: **1931 body→1220 = oracle 1220 (exact); 1945 body→1526 ≈ oracle 1527 (−1 OCR clip).** ⇒ **build/validate the modern denominator from the body we already have (a body-max sweep) — no CONTENTS-page OCR required.** Printed indices are essential only for the EARLY era (garbled body); the modern era validates its own oracle.
- **Net:** the oracle has real, localized early-era undercounts (1887/1883 confirmed); the corpus-wide 92.7% doesn't move materially yet (those are few volumes), but per-session early-era metrics against the oracle are unreliable. Parser fixes before broad trust: enforce contiguous-from-1 + monotonic run; add subject/amendment-index END markers; ±2 tolerance; modal-year quality gate. **Do NOT overwrite the oracle** — discrepancy table is for Patrick's review. Full handoff: `docs/00_Inbox/HANDOFF_5090_OPUS_2026-06-17.md`.
...
| (iii) oracle-suspect | 12 | low-N "regular" oracle rows — **UNRESOLVED** (1887 printed vol shows 185 ch vs oracle 51; see below) |
| (iv) floor over-extraction | 687 phantom / 10 sessions | precision/"garbage", NOT coverage |
- **(iii)/(iv) — RESOLVED for 1887, and it exposes an ORACLE-DENOMINATOR problem.** 2026-06-17: `production-1887` is genuinely the 1887 volume (ch53 = "Branch State Normal School in Northern California", real continuous acts to **ch 185**) — distinct from `production-1885-86` (ch53 = "Boardman claim", max 169). So **oracle 1887 = 51 is a SEVERE UNDERCOUNT (true ≈185); the floor there is REAL, not over-extraction.** Root cause: the clerk web index that (likely) seeded the oracle is unreliable — it shows "Statutes 1–51" for a 185-chapter session (surfaces only the first range link). 1885 checks out (oracle 169 == volume max 169), so it's not universal — but **the oracle denominator is suspect wherever sourced from that index, so the 92.7% carries a real error bar.** Floor-fixing stays HALTED (phantoms = real chapters). **PRIORITY FIX = re-derive per-session counts from each printed volume's actual chapter range / TOC (the authoritative internal source) and reconcile against the oracle.** Earlier note (now superseded): 2026-06-17: tried to verify via the clerk archive (1887→"1–51", 1883→"1–23") but the PRINTED VOLUME OCR contradicts it for 1887 — a continuous run of real acts to **ch 185** (distinct 169). So for 1887 either the oracle undercounts (51 ≪ ~185) OR `production-1887` is mislabeled-as-1885 (its 169 distinct == oracle 1885=169). **Cannot resolve by web index — flip-flopped twice.** Implications: (1) some "over-extraction" is actually REAL chapters → floor-fixing HALTED pending resolution; (2) the oracle denominator itself may be wrong in places, so the 92.7% has an error bar. **1883 is the cleaner case** (1883-84 = Code *amendments* volume; floor 82 likely = amendments-as-chapters, oracle 23 plausibly right). **PATH (Patrick's idea): re-derive per-volume chapter counts from each volume's printed TOC/index + actual chapter range — the authoritative internal source — rather than the clerk web index. This is the right oracle-validation + chapter-locating method and supersedes ad-hoc WebFetch.**
...
|---|---|---|
| (i) cheap text re-extraction | clean header is in `surya_text`/`doctr_text` but the parse used the garbled `consensus_text` — re-read the per-engine field, NO re-OCR | **987 / 47%** |
| (ii) position / garbled-numeral | glyph present, numeral unreadable, or body bracketed by clean neighbors → `recover_lost_header` position renumber | **799 / 38%** |
| (iii) genuine re-OCR | no engine has a usable header | **332 / 16% (UPPER BOUND)** |

- **OCR JSON for the fix:** `production-<label>/ocr_consensus/page_ocr_results.json` (dict keyed by page-string); per-page fields `tess_text, doctr_text, surya_text, consensus_text, agreement_ratio, …` — ALL retained even in raster-purged bundles.
- **TWO header eras:** early italic 1860-66 = inline `CHAP. <ROMAN>.— An Act` (Tesseract garbles the glyph, surya/doctr read the roman cleanly, consensus picked the garble); modern 1910+ = standalone `CHAPTER N.` line (all engines read well).
- **1915 (the 36% outlier) ROOT CAUSE = PARSE-STAGE FAILURE, not data loss.** Volume fully present (1922 pp), surya has 451 clean `CHAPTER N.` headers, union of 3 engines = 498/771, but the parser emitted only 278. 437/493 are cheap (i+ii). The **modern outliers 1915 + 1911 + 1941 share ONE parser/extraction fix → ~1,133 cheap chapters.**
- **EARLY `oracle_N` is INFLATED** (1860 oracle_N=455 vs max-confident 405 vs historical ~374). The early "missing" + (iii) re-OCR figures are upper bounds — **sanity-check `oracle_N` per early session before treating any (iii) as real loss.** (7th measurement-not-loss confirmation.)
- **Plan:** (1) multi-engine header re-extraction (bucket i, both eras) — biggest cheap lever, do the modern parser fix FIRST (1915→1911→1941); (2) `recover_lost_header` position-fill (bucket ii) after its Hans hardening; (3) early-era `oracle_N` sanity correction; (4) re-OCR ONLY the confirmed ~≤332 residual — defer to Patrick (heavy GPU op, and shrinking as i/ii land).
...
### ★★★★ CURRENT (2026-06-17, after recover_lost_header bucket-ii) — `chapter_vs_oracle.py` on `_chapters_all_v4.tsv`
**OCR corpus (1850–1999) = 92.7% complete (confident), biennium-correct.** 88,593 / 95,555; missing 6,962. Adds the +224 single-slot lostheader (garbled-numeral position-fill) recoveries on top of the multi-engine pass (15 multi-slot lostheader QUARANTINED, not counted). **Cumulative precision-clean/Hans-gated/0-dup recovery this run: certify +3,213, modern ME +3,413, early ME +~1,224, lostheader +224.** The remaining 6,962 = genuine re-OCR (bucket iii, deferred for Patrick — heavy GPU) + early-era FLOOR over-extraction cleanup (precision, §4) + the hardest tail.

### ★★★ prior (2026-06-17, after EARLY-ERA roman multi-engine recovery) — `_chapters_all_v3.tsv`
