...
**Gate:** B (Data Source Reconnaissance) -- Done (cc002), both modern and historical. Next: Gate C (schema). No product code yet.
...
| C | Schema (era-aware) | **Next** | Design once for the whole timeline. Synthetic `section_id` lineage surviving renumbering; `section_number_history`; **operative-date ranges** with `daterange` + GiST exclusion; **recodification events** as first-class entities (esp. Political Code dismantling 1937-1953); act-based model for the pre-code era; provenance (chaptered bill/act + source scan URL) on every version; era-aware effective-date rules. Drizzle, applied to local + Supabase. |
| D | Historical Slice Proof (RISK GATE) | Not started | The make-or-break test. One code (candidate: **Penal Code** — 1872 original surviving to today, avoids the Political-Code recodification tangle). Harvest + correct the 1872 baseline OCR to near-perfect (human-verified) -> parse to sections -> apply amendments forward several decades -> **validate against Deering's/West's history chains and the current snapshot at the seam**. Correction via constrained vision-LLM on 5090 + second-engine diff; section-number-integrity QA. If it holds to a trustworthy standard, the approach is proven. If not, we learn it cheaply. |
