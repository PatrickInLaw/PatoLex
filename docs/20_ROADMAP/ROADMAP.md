...
| D | Schema (event-sourced, domain-neutral, era-aware) | **Design done (cc002)** — see `docs/40_SCHEMA/SCHEMA_DESIGN.md`; DDL impl pending | **Decided:** append-only **event log** is the system of record (point-in-time text, current text, and the Git history are all *derived*); **domain-neutral** (enactment→provision keyed by jurisdiction+unit_type, so CA regs + a federal v2 drop in); §9605/recodification reconciliation lives in the schema, never in Git; **USLM/Akoma-Ntoso-aware** field names. Core entities: `source_document`, `enactment` (the "commit", w/ chapter_number + 3 dates), `provision` (synthetic surface-independent `provision_id` = the lineage anchor), `designation_history`, `change_event` (append-only, whole-section `new_text`, §9605 resolution metadata), `recodification` (first-class, 1872/1943). Read models: materialized `provision_version` (daterange + GiST + tsvector) for Supabase serving; Git emission from the log. Local Postgres/SQL Server (staging) + Supabase (serving). |
| E | Historical Scale-Out | Not started | **Build order (decided cc002): start at the proven 1872 codified baseline and run Method A forward, NOT cold at 1850.** First production vertical = Penal Code 1872→1991 seam (extends the validated slice); then the other codes; then the **1850-1871 pre-code era as a distinct pass** (uncodified `act_section`s) that feeds *into* the 1872 codification `recodification` event. Rationale: 1872 is proven ground (clean baseline + validated engine); the pre-code act-era is a genuinely different, still-unproven modeling problem and earns its own mini-decision/spike before going cold to 1850. The blank-slate principle defines the *complete corpus*, not the build *order*. **Source CLEAN non-Google scans (IA JP2 / Chief Clerk / HathiTrust 300dpi+), never Google Books** (the OCR blocker). OCR Tesseract 5 + qwen2.5vl ensemble + disagreement-flagging + spot-audit. Immediate bounded task: OCR the 1873-80 Chief Clerk volumes to fill the "Amendments to the Codes" gap. Model recodification events (esp. 1943 Government Code). |
