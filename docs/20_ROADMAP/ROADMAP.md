...
**Gate:** E (Historical Scale-Out) — **the production build is UNDERWAY.** Core risks (OCR quality, reconstruction method) are retired; the schema is live; the corpus is being built forward from 1850.

**As of 2026-06-02:**
- **Schema is LIVE on local PostgreSQL 16** (5080 box, DB `patolex`): migrations `0000`–`0004`, **7 tables**, with `btree_gist`, GiST exclusion constraints, `uuid_generate_v7()`, and the generated `fts_vector` all applied cleanly. (The earlier "DDL written but not applied to a live DB" state is obsolete.)
- **1850–1875 is INGESTED as the system of record: 4262 acts** (one each in `enactment` / `provision` / `change_event` / `designation_history`), across **21 source volumes**, written by the canonical `pipeline/ingest_clean.py` as **version-B multi-engine token consensus** (UTF-8 faithful, zero single-engine committed text). Consensus method on the committed rows: 4057 acts `token_majority_3` + 205 `token_majority_2`.
- **Consensus = 3 engines: Tesseract + docTR + Surya** (`consensus.py`, `N_MAX_ENGINES=3`). qwen2.5vl / GOT run as disagreement-flagging vectors only and are NEVER committed as text. *(PaddleOCR is NOT a consensus voter, despite older bake-off prose.)*
- **`provision_version = 0` and `lineage_edge = 0` — both BY DESIGN.** `provision_version` is a materialized read model (a deferred build/publish-time sweep, not yet run); `lineage_edge` is empty because the 1872 recodification edges are not yet materialized.
...
- **In flight:** the OCR campaign is extending forward past 1875; modern-format parser work (`parse_born_digital.py`, tier b) is prototyped but **not yet ingested.**
...
**As of 2026-06-08 (cc006) — live DB inventory corrects the figures above:**
- The local DB holds **35,332 enactments spanning 1850–2024** (provision 84,118; change_event 151,763; `lineage_edge` / `provision_version` still 0). The "4262 acts / 1850–1875 only" figure above is **superseded** — that was just the original historical slice.
- **Two layers coexist:** (a) **OCR-linked** session laws (`source_document_id` set), 1850–2008 = 12,552 acts — but **1876–1989 is nearly empty (~360 acts)**, which is the real un-built span; (b) **Gate F (the modern era) is LARGELY BUILT** — 22,780 acts, **1991–2024**, reconstructed from official **leginfo CAML bill-XML** (139,211 section change_events, `trust_level='official_xml'`, confidence 1.0). **=> "Gate F — Not started" below is WRONG; it is substantially done** (`pipeline/gate_f/`).
- **Gate F gaps:** sessions **1993-94, 2001-02, 2003-04** are absent and the layer ends at 2023-24. The PUBINFO archives for exactly those — plus **2025** (current `LAW_SECTION_TBL` anchor) and 1989 — were acquired 2026-06-08; parse + ingest to close the gaps and reach **current-through-2026** is in progress.
- **OCR↔Gate-F overlap (~1995–2008) is real** (same chapters in both layers, different citation keys, no collision). **Gate F (official XML) is authoritative**; the OCR for those years is the **seam-validation oracle**. Arbitration rule for the served corpus still to define.
- **Real remaining build work:** (1) ingest/fill **1876–1993 historical** (OCR), (2) finish Gate F to gap-free 1989→2026, (3) run the deferred **materialize/lineage** sweeps (`provision_version` / `lineage_edge` are still 0).
