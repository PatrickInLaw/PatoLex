...
- **PREREQUISITES to the single mass ingest (NOT separate ingests).** Before the one backup→clear→full-1850–2026-ingest→diff pass can run, finish: (1) `source_document` registration for 1877–1990 volumes, (2) `LEGISLATURE_MAP` extension past 1875-76, (3) dedup-variant resolution 1927-1965, (4) logical-key diff harness for re-ingest validation, (5) **parser fix** for the 19th-c Roman-numeral chapter headings + the chaptered_date parser bug (51 acts wrong date, correct text) — re-derive citations, (6) parse+ingest-ready the **2025 + current-2026** modern data (already acquired 2026-06-08) so the single pass reaches gap-free 1850→2026, (7) the modern-layer integrity fix (22,780 Gate F acts have NULL `source_document_id`), (8) text-quality corrections layer (Pass A + high-confidence fixes, reversible), (9) **the PROPOSITION/INITIATIVE + CONSTITUTION GATE (NEW, Patrick 2026-06-22) — an EXPLICIT gate that runs AFTER legislative work is closed out and BEFORE the single ingest.** Voter-approved initiative measures amend statutes AND the constitution outside the "Chapter N" pipeline and are currently MISSED entirely (verified — the `*_Measures.pdf`/`*_Initiative.pdf`/`*_Constitution.pdf` are on disk but never ingested). Design is pilot-tested and locked: ADOPTED measures only (defeated = a far-future "branches" project), all measure types, separate proposition-keyed parser track reusing the event-sourced `enactment→change_event` spine + a new `precedence` column, constitution built in parallel via `const_article`, two-track (probe born-digital + OCR) for post-1992. See `docs/30_SYSTEM_DESIGN/PROPOSITION_MEASURE_INGEST_DESIGN.md` §9. **NOT started yet — the OCR-era legislative recovery (99.5%, all 108 yrs mapped as of 2026-06-22) is being closed out FIRST.** When ALL of these are ready → run the one mass ingest.
...
| Role | Where | Purpose |
|------|-------|---------|
| Build / active corpus | **Local PostgreSQL 16** (`localhost:5432/patolex` on the 5080) | Pipeline ETL, OCR structuring, amendment application, reconstruction, validation, ad-hoc + Claude Code analysis. **This is where the live corpus data lives today.** Disposable, fast. |
| Serving *(planned/future)* | Supabase PostgreSQL 16 | Read-optimized published corpus the public web app reads. **Not yet active — provisioned only when the corpus is complete and validated.** |

One Drizzle schema applied to both; a publish step will promote finished data local → Supabase when the corpus is ready.
...
- Storage footprint of the full multi-version corpus + tsvector indexes (for future Supabase serving tier sizing).
