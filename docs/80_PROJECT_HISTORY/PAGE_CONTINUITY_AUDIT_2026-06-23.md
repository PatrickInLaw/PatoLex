...
- **Early-1850s not-auditable volumes -- BOUNDED, not fixable (diagnosed 2026-06-29).** The 9 `low_support` volumes 1852-1860 (3,059 pp) have **no machine-recoverable running-head page numbers**: the top-strip OCR (already upscaled) reads body `SEC. N` numbers, not page numbers, and NEITHER the top nor the bottom strip yields a consistent `printed = pdf + offset` -- the dominant implied offset is only **~2%** of reads (a real pagination shows >50%). So it is not a faintness or crop-position miss; the corner number simply isn't legibly present in these early scans. Per the lean call (not worth elaborate OCR machinery for 9 small early volumes), this is **bounded rather than fixed**: ~**3,059 pages across 9 early statute volumes remain unverified for missing leaves**. Stakes are low: these years are chapter-complete (the OCR-era chapter residual has ZERO 1850s chapters), so any undetected drop would be a mid-act body leaf, not a lost chapter. The remaining 5 not-auditable are NOT "junk stubs" (corrected 2026-06-29 -- earlier framing was wrong): `production-1883-84`, `production-1927-vol1-26chapters`, and `production-1929-vol1-28chapters` are **real, registered EXTRA-SESSION volumes** (1884 / 1926 / 1928 extra sessions) holding **unique act text NOT duplicated** in their regular siblings (e.g. 1929-28ch ch.1 = a constitutional-amendment submission, vs 1929-29ch ch.1 = the Colorado River Compact) and referenced across 8 pipeline files -- they are not auditable only because they are tiny 2-15pp fragments, NOT because they are disposable. They were investigated for cleanup and CORRECTLY KEPT (quarantine nothing -- see `STUB_CLEANUP_FINDINGS_2026-06-29.md`). The last 2 are proposition booklets (`measures-1915/1935`, out of page-audit scope). **Open follow-up:** `1927-vol1-26chapters` has 0 parsed acts despite being the registered 1926 Extra Session -- a possible parse gap, invisible to the regular-session scoreboard (extra sessions are not in the oracle); worth checking under the extra-session-completeness workstream.
