...
3. **Early-era FLOOR over-extraction (PRECISION / your "garbage" bar)** — e.g. 1865-66's *certified floor* has 463 chapters vs oracle 280 (~183 phantom: resolutions/special-acts/garble miscounted as chapters). Pre-existing, NOT introduced by recovery. **Should be cleaned before ingest** — the clearest remaining "garbage." Delicate (real-vs-phantom discrimination) — left for a supervised pass. *(A quick over-extraction scan I ran is CONFOUNDED — it conflates this with the even-year bundling artifact and oracle errors, and misses 1865-66 on a key mismatch; a clean measure needs the biennium-aware tool. Confirms this is delicate, not a night job.)*
   - **RESOLVED (2026-06-17): oracle 1887 = 51 is a SEVERE UNDERCOUNT (true ≈185).** `production-1887` is genuinely 1887 (ch53 = "Branch State Normal School", real acts to ch 185), distinct from `production-1885-86` (ch53 = "Boardman claim", max 169). The clerk web index is unreliable (shows "1–51" for a 185-ch session). ⇒ **the oracle denominator is suspect wherever it came from that index; the 92.7% has an error bar; floor-fixing HALTED (those "phantoms" are real chapters).** Fix = re-derive counts from printed-volume TOC/chapter-range, not the web index. Superseded prior note: The clerk archive page shows 1887 "Statutes 1–51" but the printed-volume OCR shows a CONTINUOUS run of real acts to **chapter 185** (distinct 169). So either (A) the clerk index is partial and 1887 really had ~185 chapters (oracle 51 = bad undercount; the floor is REAL, not over-extraction), or (B) `production-1887` is MISLABELED and holds the 1885 volume — our 1887 floor = 169 distinct, which exactly matches the oracle's **1885 = 169** (red flag). **Floor-fixing is HALTED** for these — the "phantoms" are very likely real chapters; reclassifying would delete real data. **1883 is a separate, more plausible case:** clerk 1–23 may be right (1883-84 is the Code *amendments* volume, not chapters), so its floor 82 may genuinely be amendments-mis-parsed-as-chapters. **Resolution: needs the TOC/printed-volume-based oracle re-derivation (below), Patrick's call — NOT my WebFetch guesses, which flip-flopped twice.**
